// Cleanup Duplicate Thumbnail and Cache Records - WORKING VERSION
// This script handles the ACTUAL data structure with embedded arrays in collections
// Run this script using MongoDB shell: mongosh image_viewer cleanup-duplicate-records-working.js
// 
...
// Check if this is a dry run - simple approach
var isDryRun = false;

// For mongosh with --eval, the variable should be set externally
// For testing, you can manually set it to true here:
// isDryRun = true;
...
collectionsCursor.forEach(function(coll) {
    ...
    if (thumbnailDuplicates.length > 0 || cacheDuplicates.length > 0) {
        ...
        // SAFETY CHECK: Only check for negative counts (removed ratio limit)
        ...
        print("   📊 Duplicate ratios - Thumbnails: " + Math.round(thumbnailRemovalRatio * 100 * 10) / 10 + "%, Cache: " + Math.round(cacheRemovalRatio * 100 * 10) / 10 + "%");
...
if (isDryRun) {
    ...
    print("   To run for real, set isDryRun = false in the script");
} else {
    print("\n✅ Cleanup completed!");
}
